...
function extractProjectOptionalDependencies(
  pyproject: JsonMap,
): PackageDependency[] {
  let project = pyproject.project as JsonMap;
...
function extractHatchDependencies(pyproject: JsonMap): PackageDependency[] {
  let tool = pyproject.tool as JsonMap;
  let hatch = tool?.hatch as JsonMap;
  let env = hatch?.env as JsonMap;
  let requires = env?.requires as string[];
...
export async function extractPackageFile(
  ...
  return {
    deps: [
      ...
      ...extractHatchDependencies(pyproject),
    ],
  };
}
