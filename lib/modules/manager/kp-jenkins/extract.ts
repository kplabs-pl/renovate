...
function extractPythonDependency(dependencyText: string, name: string): PackageDependency | null {
...
function extract_withPipxInstalled(jenkinsfile: string): PackageDependency[] {
    ...
    while ((requirementsMatch = requirementsRegex.exec(jenkinsfile)) !== null) {
        if (requirementsMatch?.groups?.requirements.startsWith('[')) {
            ...
            while ((requirementMatch = requirementRegex.exec(requirementsMatch?.groups?.requirements)) !== null) {
                const packageDependency =  extractPythonDependency(requirementMatch[0], 'withPipxInstalled');
            ...
            const packageDependency =  extractPythonDependency(requirementsMatch?.groups?.requirements, 'withPipxInstalled');
            if (packageDependency === null) {
                continue;
            }
...
export async function extractPackageFile(
    ...
    return {
        deps: [
            ...extract_withPipxInstalled(content),
        ],
    };
}
